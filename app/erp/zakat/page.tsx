        ...
        {STAGES.map((stage) => {
          ...
          return (
            <div key={stage} className="bg-paper p-5">
              ...
              <div className="num mt-3 text-[15px] text-ink-2">{rupees(sum)}</div>
            ...
            <article key={r.id} className="grid gap-5 bg-paper p-5 lg:grid-cols-[1.7fr_auto] lg:gap-8">
              <div>
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className="num text-[12px] font-semibold text-ink">{r.reference}</span>
                ...
                <p className="mt-3 max-w-2xl text-[14px] leading-[1.65] text-ink-2">{r.details}</p>
              </div>
              ...
              {ZAKAT_REQUESTS.filter((r) => r.status !== "PENDING").map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <Td>
                      {label(r.category)}
                      {r.isRecurring && <span className="block text-[11px] text-ink-3">standing</span>}
                    </Td>
                    ...
                    <Td className="whitespace-nowrap text-ink-2">
                      {ap ? ap.fullName : "—"}
                      {r.approverMemberId && (
                        <span className="num block text-[11px] text-ink-3">{r.approverMemberId}</span>
                      )}
                    </Td>
